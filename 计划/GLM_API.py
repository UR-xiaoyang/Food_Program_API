...
async def GLM_stream(request: Request, messages: str = Query(...), credentials: HTTPAuthorizationCredentials = Depends(安全)):
    ...
    user_info = 查询个人信息(token_data['sub'])
    user_Data = f"年龄：{user_info[2]}, 性别：{user_info[3]}, 体重：{user_info[4]}, 身高：{user_info[5]}, 健康状态：{user_info[6]}, 饮食偏好：{user_info[7]}"
